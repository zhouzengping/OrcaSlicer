...
PressureEqualizer::PressureEqualizer(const Slic3r::GCodeConfig &config) : m_use_relative_e_distances(config.use_relative_e_distances.value)
{
    ...
    const double max_volumetric_extrusion_rate_slope = get_value_at(
        config, config.max_volumetric_extrusion_rate_slope, ConfigFlowDomain::Process, m_current_extruder);
    if (max_volumetric_extrusion_rate_slope > 0) {
        m_max_volumetric_extrusion_rate_slope_positive = float(max_volumetric_extrusion_rate_slope) * 60.f * 60.f;
        m_max_volumetric_extrusion_rate_slope_negative = float(max_volumetric_extrusion_rate_slope) * 60.f * 60.f;
        m_max_segment_length = float(get_value_at(
            config, config.max_volumetric_extrusion_rate_slope_segment_length, ConfigFlowDomain::Process, m_current_extruder));
        m_extrusion_rate_smoothing_external_perimeter_only = get_value_at(
            config, config.extrusion_rate_smoothing_external_perimeter_only, ConfigFlowDomain::Process, m_current_extruder);
    }
