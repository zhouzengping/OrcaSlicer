      ...
      m_g1_f_pattern(R"(G1 F([0-9]+))")
{
    ...
    for (unsigned int tool : tools_used) {
        ...
        if (m_config.adaptive_pressure_advance.get_at(tool) &&
            get_value_at(m_config, m_config.enable_pressure_advance, ConfigFlowDomain::Filament, tool)) {
        ...
        if (line.find("; PA_CHANGE") == 0) { // prune lines quickly before running regex check as regex is more expensive to run
            if (std::regex_search(line, m_match, m_pa_change_pattern)) {
                ...
                if(!interpolator){ // Tool not found in the interpolator map
                    ...
                    predicted_pa = get_value_at(m_config, m_config.enable_pressure_advance, ConfigFlowDomain::Filament, m_last_extruder_id) ? get_value_at(m_config, m_config.pressure_advance, ConfigFlowDomain::Filament, m_last_extruder_id) : 0;
