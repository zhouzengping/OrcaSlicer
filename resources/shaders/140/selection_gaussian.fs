...
float sampleSymmetricPair(float offset)
{
    ...
    return texture(source_texture, tex_coord + uvOffset).a +
           texture(source_texture, tex_coord - uvOffset).a;
}

void main()
{
    float blurred = texture(source_texture, tex_coord).a * center_weight;
    ...
    frag_color = vec4(0.0, 0.0, 0.0, blurred);
}
