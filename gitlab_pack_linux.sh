#!/usr/bin/env bash
#
# gitlab_pack_linux.sh — Build Snapmaker_Orca Linux packages (AppImage + Flatpak)
#                        using GitLab as the code source.
#
# Default with no args: -F all -c -dsir -j 3 -I
#
# Branch and version:
#   ./gitlab_pack_linux.sh -v "2.3.3" -b "dev_2.3.3_alves_packaged"
#   → fetch branch from GitLab + force -F all -c -dsir -j 3 -I
#   → write version into source (version.inc / metainfo) and artifact names
#   → copy artifacts to $SCRIPT_DIR
#
# AppImage: host gitlab_build/ compile + appimagetool
...
REPO_URL="http://${GITLAB_BASE_URL}/${GITLAB_GROUP}/OrcaSlicer.git"
...
GIT_BRANCH=""               # -b: GitLab branch name
JOBS_EXPLICIT=false

HOST_BUILD_DIR="gitlab_build"
...
FLATPAK_BUILD_DIR="gitlab_flatpak_pack"
FLATPAK_CACHE_DIR=".gitlab_flatpak-builder-pack"
...
REPO_DIR=""                 # build root (-b: $SCRIPT_DIR/gitlab_$BRANCH/OrcaSlicer)
...
Version / branch (aligned with gitlab_build_release_macos.sh):
  ...
  -b, --branch NAME        Fetch GitLab branch, then force full pack:
                           -F all -c -dsir -j 3 -I
                           Source dir:  \$SCRIPT_DIR/gitlab_\$BRANCH/OrcaSlicer
...
Build:
  -c, --clean              Pass -c to gitlab_build_linux.sh (clean host build cache)
  -dsir, --dsir            One-shot compile: gitlab_build_linux.sh -dsr (default on)
      --no-dsir            Skip host compile; package existing binary (AppImage)
  -D, --build-dir DIR      Host build directory [default: gitlab_build]
      --build-if-missing   Run gitlab_build_linux.sh -s if binary is missing

AppImage:
  -k, --keep-package       Keep gitlab_build/package working directory
  -A, --appimagetool PATH  Local appimagetool path
      --no-network         Do not download appimagetool/type2 runtime

Flatpak:
      --flatpak-build-dir DIR   flatpak-builder output directory [default: gitlab_flatpak_pack]
      --flatpak-cache-dir DIR   flatpak-builder state cache [default: .gitlab_flatpak-builder-pack]
...
# ---------------- Git repo setup (aligned with gitlab_build_release_macos.sh) ----------------

prepare_git_repo() {
    local branch="$1"
    local work_dir="$SCRIPT_DIR/gitlab_$branch"
...
pack_appimage() {
    ...
    if [[ "$FULL_DSR" == true ]]; then
        echo -e "${BLUE}One-shot compile: ./gitlab_build_linux.sh -dsr$( [[ "$CLEAN_BUILD" == true ]] && echo ' -c' )${NC}"
        local BL_ARGS=(./gitlab_build_linux.sh -d -s -r)
    ...
    if [[ -z "$BIN_PATH" && "$DO_BUILD" == true ]]; then
        echo -e "${YELLOW}Binary not found; running ./gitlab_build_linux.sh -s ...${NC}"
        local BL_ARGS=(./gitlab_build_linux.sh -s)
        [[ "$CLEAN_BUILD" == true ]] && BL_ARGS+=(-c)
        [[ -n "$JOBS" ]] && BL_ARGS+=(-j "$JOBS")
        "${BL_ARGS[@]}"
        BIN_PATH="$(locate_host_binary || true)"
    ...
    if [[ -z "$BIN_PATH" ]]; then
        ...
        echo -e "${YELLOW}Default is -dsir auto-build; use --build-if-missing or run gitlab_build_linux.sh -dsi manually${NC}"
        exit 1
    fi
